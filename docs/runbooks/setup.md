...
| ---------- | ------ | --- |
| Secrets    | Read and write | `AZURE_CLIENT_ID` on app/tests repos; `github_actions_secret` |
| Variables  | Read and write | `AKS_*` on the app repo; Environment variables `ARM_*` / `TF_STATE_*` on this repo |
| Environments | Read and write | Create GitHub Environments `dev` / `staging` / `prod` for plan/apply |
...
| Tenant       | **Cloud Application Administrator** on `gha_terraform` | Manual/PIM after first apply — CI must manage Entra apps |
...
The PR-env UAMI (`gha_pr_env`) is created on **dev only** (Flux preview and wait-ready CI target `aks-bjjeire-dev-sdc-01`; staging/prod overlays omit `bjj-eire-preview` and Kyverno `deny-ephemeral-envs` rejects those namespaces). Prod fail-closed: `aks_local_account_disabled = true`, a non-empty `aks_api_server_authorized_ip_ranges` unless the cluster is private, and Playwright / Cloudflare tests-token flags off. Applying prod without authorized CIDRs will error at plan. Do **not** set `aks_private_cluster_enabled = true` while plan/apply and PR-env wait-ready run on GitHub-hosted runners — they need a public API.

Node subnets disable default outbound access and send egress through a NAT Gateway (`aks_outbound_type = userAssignedNATGateway`). The system pool lands on the **system** subnet; apps/runners stay on **workload**. Key Vault gets a private endpoint in `PrivateEndpointSubnet` plus `privatelink.vaultcore.azure.net` so External Secrets stays on the VNet. Leave `kv_public_network_access_enabled = true`. The prod example uses `kv_network_acls.default_action = Deny` with operator/CI CIDRs in `ip_rules`; empty `ip_rules` with Deny blocks both laptop apply and GitHub-hosted apply.

Secrets stay in the environment, not in tfvars. Grafana and MongoDB passwords are generated on apply and written to Key Vault (`grafana-admin-password`, `bjj-mongodb-root-password`). To rotate Grafana: `terraform taint random_password.grafana_admin` then apply.

```bash
export TF_VAR_cloudflare_api_token='<token>'
export TF_VAR_github_app_private_key="$(cat /path/to/bjjeire.private-key.pem)"
export TF_VAR_ghcr_pat='<github-PAT-with-read:packages>'
# required on dev (Flux PR ResourceSetInputProvider)
export TF_VAR_github_preview_pat='<fine-grained-PAT>'
```

### GitHub Actions plan/apply (after the first laptop apply)

The first apply of this stack creates `gha_terraform` (OIDC to `environment:<env>` and `refs/heads/main`), the GitHub Environment named after `var.environment`, and these Environment **variables**: `ARM_CLIENT_ID`, `ARM_TENANT_ID`, `ARM_SUBSCRIPTION_ID`, `TF_STATE_RG`, `TF_STATE_ACCOUNT`.

The GitHub App also needs **Environments: Read and write** (and Variables: Read and write) on this repository so those resources can be managed.

Then set these **Environment secrets** once in the GitHub UI (Settings → Environments → `dev` / `staging` / `prod`). They are never written by Terraform:

| Secret | Maps to |
| ------ | ------- |
| `TF_VAR_CLOUDFLARE_API_TOKEN` | `TF_VAR_cloudflare_api_token` |
| `TF_VAR_GITHUB_APP_PRIVATE_KEY` | `TF_VAR_github_app_private_key` |
| `TF_VAR_GHCR_PAT` | `TF_VAR_ghcr_pat` |
| `TF_VAR_GITHUB_PREVIEW_PAT` | `TF_VAR_github_preview_pat` (dev) |
| `TF_VAR_PRIVATE_EMAIL` | `TF_VAR_private_email` |
| `TF_VAR_GITHUB_TOKEN` | `TF_VAR_github_token` (until the App can write Actions secrets) |

Commit a **secret-free** `environments/<env>/terraform.tfvars` (the gitignore allows it). CI reads that file; secrets come from the Environment.

On staging/prod, add required reviewers on the GitHub Environment (or set `github_environment_reviewer_logins` and re-apply). Restrict those environments to the `main` branch (already set by this stack).

**Break-glass laptop apply** remains the commands below. Prefer CI once the Environment is populated.
