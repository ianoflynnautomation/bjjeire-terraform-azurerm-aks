variable "cloudflare_nsg_name_suffix" {
  type        = string
  default     = "-cloudflare-ingress-nsg"
  description = "Suffix appended to var.aks_cluster_name to form the Cloudflare ingress NSG name."
}

variable "cloudflare_nsg_allow_https_priority" {
  type        = number
  default     = 100
  description = "Priority of the Allow rule for Cloudflare IPs on 443. Lower number = higher priority."
}

variable "cloudflare_nsg_allow_http_priority" {
  type        = number
  default     = 110
  description = "Priority of the Allow rule for Cloudflare IPs on 80."
}

variable "cloudflare_nsg_deny_https_priority" {
  type        = number
  default     = 4095
  description = "Priority of the Deny rule that blocks the rest of the Internet on 443."
}

variable "cloudflare_nsg_deny_http_priority" {
  type        = number
  default     = 4096
  description = "Priority of the Deny rule that blocks the rest of the Internet on 80."
}

variable "cloudflare_nsg_https_port" {
  type        = string
  default     = "443"
  description = "Destination port for HTTPS rules (Cloudflare → origin)."
}

variable "cloudflare_nsg_http_port" {
  type        = string
  default     = "80"
  description = "Destination port for HTTP rules (Cloudflare → origin, redirected to HTTPS upstream)."
}

variable "cloudflare_nsg_protocol" {
  type        = string
  default     = "Tcp"
  description = "Transport protocol for Cloudflare ingress rules."
}
